...
// Uses PMTiles for choropleth rendering in exports
...
export function ExportMap({ filteredData, selectedMetric, regionScope, onRenderComplete }: ExportMapProps) {
  ...
  // Create lookup map from filtered data
  ...
  useEffect(() => {
    ...
    const mapConfigs: MapConfig[] = [
      ...
      ...(regionScope === 'national' && alaskaZips.size > 0 ? [
        { key: 'alaska', ref: alaskaMapRef, center: [-152, 64] as [number, number], zoom: 2.5 }
      ] : []),
      ...(regionScope === 'national' && hawaiiZips.size > 0 ? [
        { key: 'hawaii', ref: hawaiiMapRef, center: [-157, 21] as [number, number], zoom: 5 }
      ] : [])
    ];
    ...
    mapConfigs.forEach(({ key, ref, center: mapCenter, zoom: mapZoom }) => {
      ...
      map.on('load', () => {
        ...
        // Set feature states for choropleth coloring after tiles load
        map.once('idle', () => {
